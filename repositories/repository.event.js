...
async function updateEvent(req, res) {
  const { title, description, year, period, month, day, country, city, id } =
    req.body;
...
async function country(req, res) {
  ...
  try {
    const result = await pool.query(
      "SELECT * FROM historical_events WHERE country = $1",
      [country]
    );
...
async function paginate(req, res) {
  ...
  try {
    ...
    const result = await pool.query(
      "SELECT * FROM historical_events LIMIT $1 OFFSET $2",
      [pageSize, pageReal]
    );
